...
def info_nce_mi(
    ...
    *,
    ...
    def bound(xe, ye):
        ...
        return math.log(2.0) + float((positive - torch.logsumexp(torch.stack([positive, negative]), dim=0)).mean())
...
def _equal_count_bins(values, bins):
    ...
    midranks = np.cumsum(counts) - counts / 2.0
    assignments = np.minimum((midranks * bins / len(values)).astype(np.int64), bins - 1)
    return assignments[inverse], min(bins, len(unique_values))
...
def interaction_information(x, y, labels, *, steps=600, hidden=64, seeds=(0, 1, 2, 3, 4), device="cpu"):
    ...
    left = estimate_mi(x, labels[:, None], methods=("dv",), seeds=seeds, steps=steps, hidden=hidden, device=device)[
        "dv"
    ]["median"]
    right = estimate_mi(y, labels[:, None], methods=("dv",), seeds=seeds, steps=steps, hidden=hidden, device=device)[
        "dv"
    ]["median"]
    joint_input = np.concatenate([x, y], axis=1)
    joint = estimate_mi(
        joint_input, labels[:, None], methods=("dv",), seeds=seeds, steps=steps, hidden=hidden, device=device
    )["dv"]["median"]
    return {
...
def collect_embeddings(model, dataset, batch_size, *, input_res=None, device=None, num_workers=0, amp_dtype=None):
    ...
    with torch.no_grad():
        for x, views, target in DataLoader(
            ...
            with torch.autocast(
                device.type,
                dtype=getattr(torch, amp_dtype or "float16"),
